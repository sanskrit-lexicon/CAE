"""
Markup fixer + audit for cae.txt (CAE).
Counterpart of PWG#175, PWK#113, and siblings.

CAE-specific notes: 0 whitespace hits.

Inputs: ../../../csl-orig/v02/cae/cae.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "cae" / "cae.txt", HERE / "cae.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "cae_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(lex|ab|lang)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for cae.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"CAE markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against cae.txt.\n")
        f.write(f"Paired tags: <lex>, <ab>, <lang>\n")
        f.write(f"Notes: 0 whitespace hits.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <lex>, <ab>, <lang>\n\n")
